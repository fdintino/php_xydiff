...
if test "$PHP_XYDIFF" != "no"; then
	...
	PHP_INCLUDE_DIR=`$PHP_CONFIG --include-dir 2>/dev/null`
	PHP_ADD_INCLUDE($PHP_INCLUDE_DIR/Zend)
	PHP_ADD_INCLUDE($PHP_INCLUDE_DIR/TSRM)
	PHP_ADD_INCLUDE($PHP_INCLUDE_DIR/main)
